...
(function (factory) {
  ...
  var dropdown = renderer.create('<div class="dropdown-purchase">', function ($node, options) {
  ...
  var dropdownCheck = renderer.create('<div class="dropdown-purchase note-check">', function ($node, options) {
    var markup = $.isArray(options.items) ? options.items.map(function (item) {
      var value = (typeof item === 'string') ? item : (item.value || '');
      var content = options.template ? options.template(item) : item;
