        ...
        <div class="col-sm-12">
            <div class="main-content-header">
                <div class="pull-left header-title">
                    <h4>字典管理</h4>
                    <div class="small-title">字典列表</div>
                </div>
            </div>
        </div>
    </div>
                ...
                <form id="member_list_searchForm" name="member_list_searchForm" class="form-inline list-search" onsubmit="return false">
                    <div class="form-group">
                        <select class="form-control" name="search_type">
                            <option value="username" >字典名</option>
...
<script>
    var member_insert_modal_url = '/sys/other/dictionary/add'; //新增注册url
    var member_list_view_url = '/sys/other/dictionary/view/'; //查看信息URL
    var member_list_update_url = '/sys/other/dictionary/update/'; //更新信息URL
    var member_list_delete_url = '/sys/other/dictionary/delete'; //删除URL

    var member_list_table; //用户表对象
    var member_list_table_id = 'member_list_table'; //用户表id

    $(function () {
        selectMenuByID("admin_dictionary");
    ...
    function member_list_loadData() {
        member_list_table = $('#' + member_list_table_id).bootstrapTable({
            ...
            url : "/sys/other/dictionary/findList",
            pagination : true,
            search : false,
            showRefresh : false,
            showToggle : false,
