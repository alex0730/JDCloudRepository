        ...
        <div class="col-sm-12">
            <div class="main-content-header">
                <div class="pull-left header-title">
                    <h4>角色管理</h4>
                    <div class="small-title">角色列表</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-sm-12">
            <div class="pull-left">
                <button class="btn list-btn btn-danger " type="button"
                    ...
                    <i class="fa fa-fw fa-times"></i>删除角色
                </button>
            </div>
                ...
                <form id="menu_list_searchForm" name="menu_list_searchForm" class="form-inline list-search"
                    ...
                    <div class="form-group">
                        <select class="form-control" id="search_type" name="search_type">
                            <option value="cn_name" selected>角色名称</option>
...
<script>
    ...
    var menu_insert_modal_url = '/sys/auth/role/add'; //新增注册url
    var menu_list_use_url = '/sys/auth/role/use/';
    var menu_list_update_url = '/sys/auth/role/update/'; //更新信息URL
    var menu_list_delete_url = '/sys/auth/role/delete'; //删除URL

    var menu_list_table; //用户表对象
    var menu_list_table_id = 'menu_list_table'; //用户表id

    $(function () {
        selectMenuByID("admin_role");
        menu_list_loadData();
    });
    //显示列
    var menu_list_table_columns = [
        ...
        },
        {
            ...
            formatter: function (value, row, index) {
                ...
                result += '<button class="btn btn-xs btn-success user-status" dataid="'+row.id+'" status="'+row.status+'">删除</button>&nbsp';
                // if (row.status === 1) {
                //     result += '<button class="btn btn-xs btn-primary" onclick="openOrCloseConfirm(\'确认要启用角色吗？\',\'' + menu_list_use_url + '\',\'' + row.id + '\',\'' + menu_list_table_id + '\',\'true\')">启用</button>&nbsp;';
                // } else {
                //     result += '<button class="btn btn-xs btn-primary" onclick="openOrCloseConfirm(\'确认要停用角色吗？\',\'' + menu_list_use_url + '\',\'' + row.id + '\',\'' + menu_list_table_id + '\',\'false\')">停用</button>&nbsp;';
                // }
                return result;
            }
            //格式化
