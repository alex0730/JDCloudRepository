        ...
        <div class="col-sm-12">
            <div class="main-content-header">
                <div class="pull-left header-title">
                    <h4>库存周转规则</h4>
                    <div class="small-title">库存周转规则列表</div>
                </div>
            </div>
        </div>
    </div>
                ...
                <form id="member_list_searchForm" name="member_list_searchForm" class="form-inline list-search" onsubmit="return false">
                    <div class="form-group">
                        <select class="form-control" name="search_type">
                            <option value="username" >库存周转规则名</option>
...
<script>
    var member_insert_modal_url = 'base/rule/stockturn/add'; //新增注册url
    var member_list_view_url = 'base/rule/stockturn/view/'; //查看信息URL
    var member_list_update_url = 'base/rule/stockturn/update/'; //更新信息URL
    var member_list_delete_url = 'base/rule/stockturn/delete'; //删除URL

    var member_list_table; //用户表对象
    var member_list_table_id = 'member_list_table'; //用户表id

    $(function () {
        selectMenuByID("admin_stockturn");
    ...
    function member_list_loadData() {
        member_list_table = $('#' + member_list_table_id).bootstrapTable({
            ...
            url : "/base/rule/stockturn/findList",
            pagination : true,
            search : false,
            showRefresh : false,
            showToggle : false,
