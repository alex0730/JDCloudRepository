        ...
        <div class="col-sm-12">
            <div class="main-content-header">
                <div class="pull-left header-title">
                    <h4>人工建立订单</h4>
                    <div class="small-title">出库订单列表</div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    var member_insert_modal_url = '/putout/neworder/add'; //新增注册url
    var member_list_view_url = '/putout/neworder/view/'; //查看信息URL
    var member_list_update_url = '/putout/neworder/update/'; //更新信息URL
    var member_list_delete_url = '/putout/neworder/delete'; //删除URL

    var member_list_table; //用户表对象
    var member_list_table_id = 'member_list_table'; //用户表id

    $(function () {
        selectMenuByID("admin_neworder");
    ...
    function member_list_loadData() {
        member_list_table = $('#' + member_list_table_id).bootstrapTable({
            ...
            url : "/putout/neworder/neworder/findList",
            pagination : true,
            search : false,
            showRefresh : false,
            showToggle : false,
